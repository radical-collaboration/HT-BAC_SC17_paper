...
The ESMACS protocol contains replicas \jhanote{I think replica is being used
incorrectly. replica means copy. copy of what? that remains undefined here
and other places replica is used.} which perform a sequence of simulation
stages for a given physical system. Each pipeline in ESMACS consists of a
sequence of stages. Each stage consists of a task that performs unique
...
for the ESMACS protocol. Each stage is composed of a single task with a set
...
maintain temporal order during execution.\mtnote{There is some repetition in this paragraph about pipelines/stages/tasks and ESMACS\@.}
...
EnTK\@. \st{ESMACS consists of replicas where each replica maps to a
pipeline.} EnTK converts the set of pipelines into a set of tasks called
