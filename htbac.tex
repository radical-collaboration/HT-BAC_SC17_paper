...
replicas. \jhanote{I don't think this is a suitable description of
...
\mtnote{I am not sure I understand this sentence: to what do we
assign one or more replica?} \jdnote{better?}. We express the application
...
API and programming model \jhanote{What programming model? EnTK's programming model?} allow HTBAC to express the workflows associated
...
The concept of % an 
ensemble in the ESMACS and TIES \mtnote{and TIES? You mentioned it above. At
some point we will have to transition to ESMACS only but the transition needs
to be justified.} protocol maps directly to a set of pipelines in EnTK, where
each replica \jhanote{replica of what?} maps to a single pipeline. As defined
earlier by the ESMACS protocol, each replica is assigned a series of
simulation steps. \jhanote{Achtung: simulation steps will be confused with the number of
steps for an MD simulation} EnTK interprets these steps as a workflow of stages and
...
The patterns \jhanote{what is a pattern within a pipeline? a pipeline is a pattern by some accounts?} within pipelines are identical and describe an ensemble of replica
