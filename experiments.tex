...
execute all the tasks concurrently all the time but we always get enough cores
to run at least one task from each concurrent pipeline. Alternatively, at
every point in time, there are enough cores so that pipelines do not compete
for the same core.}\jhanote{I agree we do not run all the tasks concurrently.}

At every point in runtime, there are enough cores so that pipelines do not compete for the same core and hence allows consecutive stages to be able to start without delay due to a lack of resources. 
