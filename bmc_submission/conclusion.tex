...
protocol scales?}\jdnote{done} \jhanote{I don't think the protocol scales, but
our implementation of the protocol. Please see changed version. } This permits
a time-to-solution that is essentially invariant of the number
\jhanote{we should replace "size" with "number"} of candidate ligands, as well
as the type and number of protocols concurrently employed.
...
% that is'} \jdnote{just reduced it to time-to-solution instead of rapid time-
% to-solution}
...
platforms, while providing immediate benefits of scale and support for a range
of different application workflows. Thus, HTBAC represents an important
